...
// Robust loader: local vendor first, then CDN; WASM path matches JS base
async function ensureSql(){
  if (SQL) return SQL;
  const candidates = [
    relFromHere('../../vendor/sql-wasm.js'),
    'https://cdn.jsdelivr.net/npm/sql.js@1.8.0/dist/sql-wasm.js',
    'https://cdnjs.cloudflare.com/ajax/libs/sql.js/1.8.0/sql-wasm.js',
  ];
  let base = null;
...
function renderTable(container, result){
  container.innerHTML = '';
  // If we don't have a tabular result (no SELECT), show the text dump:
  if (!result || !result.columns) {
    const pre = document.createElement('pre');
    pre.textContent = lastOutput || '(no output)';
    container.appendChild(pre);
...
export async function run(){
  try{
    ...
    // Execute full script; collect result sets
    const results = db.exec(code);

    // Build text dump (used for Send)
    lastOutput = '';
    if (!results.length) {
      lastOutput = '(no result sets)';
    } else {
      results.forEach((res, i) => {
        ...
        res.values.forEach(row => {
          lastOutput += row.map(v => v==null ? 'NULL' : v).join('\t') + '\n';
        });
        lastOutput += '\n';
      });
    }

    // Show last SELECT as table (or text if none)
    const last = results[results.length - 1];
    renderTable(sqlout, last);
