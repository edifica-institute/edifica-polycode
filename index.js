  ...
  try {
    ...
    if (USE_DOCKER) {
      const compileCmd = [
        ...
        "-v", `${jobDir}:/workspace:rw`, "-w","/workspace",
        "oc-c:gcc","bash","-lc",
        `shopt -s nullglob; files=( *.c ); if (( \${#files[@]} )); then gcc -O2 -pipe -o main "\${files[@]}" 2>&1; else echo "No .c files"; fi; true`
      ];
      const out = await execCapture("docker", compileCmd);
      compileLog = out.stdout;
      diagnostics = parseGcc(compileLog);
      ok = diagnostics.every(d => d.severity !== "error" && d.severity !== "fatal error");
    } else {
      const script = `cd "${jobDir}"; shopt -s nullglob; files=( *.c ); ` +
